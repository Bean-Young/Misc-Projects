...
class CoachTest:
    def __init__(self,opts):
        ...
        self.decoder.load_state_dict(torch.load(self.opts.stylegan_weights,map_location='cuda')['g_ema'],strict=False)
        ...
        self.mapper.load_state_dict(torch.load(opts.mapper_weight,map_location='cuda')['mapper_state_dict'])
        ...
        self.arcface.load_state_dict(torch.load(opts.ir_se50_weights,map_location='cuda'))
        self.id_loss=IDLossExtractor(self.opts,self.arcface).to(self.device).eval()
        #tensor dataset related
        self.latent=torch.load(opts.latent_path,map_location='cuda')
    ...
    def load_e4e(self):
        ...
        ckpt = torch.load(self.opts.e4e_model_weights, map_location='cuda')
    ...
    def single_pwd_loop(self):
        ...
        with torch.no_grad():
            for j,(latent_batch,image_batch) in enumerate(zip(self.latent_loader,self.image_loader)):
                ...
                save_batch[:,:,0:h,w:2*w]=x_ori.cuda()
                ...
                save_batch[:,:,0:h,2*w:3*w]=x_enc.cuda()
                save_batch[:,:,h:2*h,w:2*w]=x_recover.cuda()
                save_batch[:,:,h:2*h,2*w:3*w]=x_wrong.cuda()
    ...
    def multi_pwd_loop(self):
        ...
        with torch.no_grad():
            for j,(latent_batch,image_batch) in enumerate(zip(self.latent_loader,self.image_loader)):
                ...
                save_batch[:,:,0:h,0:w]=x_ori_real
                save_batch[:,:,h:h+h,0:w]=x_ori.cuda()
                ...
                for pwd_idx in range(len(self.w_pwd_list)):
                    ...
                    save_batch[:,:,0:h,(1+pwd_idx)*w:(2+pwd_idx)*w]=x_enc.cuda()
                    save_batch[:,:,h:2*h,(1+pwd_idx)*w:(2+pwd_idx)*w]=x_recover.cuda()
                    save_batch[:,:,2*h:3*h,(1+pwd_idx)*w:(2+pwd_idx)*w]=x_wrong.cuda()
