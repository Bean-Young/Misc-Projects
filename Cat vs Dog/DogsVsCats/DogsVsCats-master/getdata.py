...
import random
from Image_preprocessing import transformos
# 默认输入网络的图片大小
IMAGE_SIZE = 224
...
class DogsVSCatsDataset(data.Dataset):      # 新建一个数据集类，并且需要继承PyTorch中的data.Dataset父类
    def __init__(self, mode, dir):          # 默认构造函数，传入数据集类别（训练或测试），以及数据集路径
        ...
        if self.mode == 'train':            # 训练集模式下，需要提取图片的路径和标签
            dir = dir + '/sets/train/'           # 训练集路径在"dir"/train/
            ...
            dir = dir + '/cat_dog/train/test/'            # 测试集路径为"dir"/test/
            random_list_cat = random.sample(range(10000), 5000)
            random_list_dog = random.sample(range(10000), 5000)
            for file in os.listdir(dir):
                name = file.split(sep='_')
                if name[0]=='cat':
                    if name[1] in random_list_cat:
                        self.list_img.append(dir + file)
                        self.data_size += 1
                        self.list_label.append(0)
                else:
                    if name[1] in random_list_dog:
                        self.list_img.append(dir + file)
                        self.data_size += 1
